...
class GIFTrainer(Trainer):
    ...
    def get_grad(self, data, model):
        ...
        neg_edge_index = negative_sampling(
            ...
            num_neg_samples=int(data.sdf_mask.sum()),
        ).long()     # ← cast to LongTensor
        ...
        # prepare integer edge‐index tensors
        pos_dr = data.train_pos_edge_index[:, data.dr_mask].long()   # edges to keep
        pos_df = data.train_pos_edge_index[:, data.df_mask].long()   # edges to delete

        logits = model.decode(out1, pos_dr, neg_edge_index)
        label  = get_link_labels(pos_dr, neg_edge_index)

        loss = F.binary_cross_entropy_with_logits(logits, label, reduction='mean')
        # loss = F.nll_loss(out1, data.y, reduction='sum')

        logits = model.decode(out1, pos_df, neg_edge_index)
        label  = get_link_labels(pos_df, neg_edge_index)
        ...
        logits = model.decode(out2, pos_df, neg_edge_index)
        label  = get_link_labels(pos_df, neg_edge_index)
